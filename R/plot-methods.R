...
if(plot_legend){
				...
				if(forecast | backcast){
					...
					plot(ts,ylim=c(min(ts,na.rm=TRUE),max(ts,na.rm=TRUE)*ytop),xlab=xlab,ylab=ylab,main=main,lwd=lwd_original,col=col_original,lty=lty_original)
					...
					plotFbcast(object=object,showCI=showCI,
							...
							ylab=ylab,xlab=xlab,ylim=ylim,showWarnings=showWarnings)	
				}
				}else	
					plot(ts,ylim=c(min(ts,na.rm=TRUE),max(ts,na.rm=TRUE)*ytop),xlab=xlab,ylab=ylab,main=main,lwd=lwd_original,col=col_original,lty=lty_original)
...
if(is.null(showOut) &! showAllout){
	...
	if(forecast | backcast){
		plotFbcast(object=object,showCI=showCI,
				...
				ytop=ytop,
				col_line=NA,
				ylim=ylim,showWarnings=FALSE,type = "n", axes = FALSE, ann = FALSE)	
	}else{
	...
	points(out.trend.x[grep(out[1],out.trend.x)],out.trend.y[grep(out[1],out.trend.x)],col=col.out,pch=pch.out,cex=cex_out,lwd=lwd_out)
					...
					if(forecast | backcast){
						plotFbcast(object=object,showCI=showCI,
										...
										ylim=ylim,type = "n", axes = FALSE, ann = FALSE)	
					}else{
...
setMethod(f='plot',
    ...
    definition=function(x,y=NULL,original=TRUE,sa=FALSE,trend=FALSE,
      ...
      plot(x@x12Output,y=NULL,original=original,sa=sa,trend=trend,
		  ...
		  showLine=showLine,col_line=col_line,lty_line=lty_line,ylim=ylim)      
    }
)
...
setMethod(
		...
		signature=signature(object = "x12Output"),definition=function(object,showCI=TRUE,
				...
				points_bc=FALSE,points_fc=FALSE,points_original=FALSE,
				showLine=FALSE,col_line="grey",lty_line=3,
				ylab="Value",xlab="Date",ylim=NULL,showWarnings=TRUE,...)
		{
			...
			if((!forecast &! backcast) | (forecast && is.na(fc[1]) &! backcast) | (backcast && is.na(bc[1]) &! forecast) | (forecast && is.na(fc[1]) && backcast && is.na(bc[1]))){
				if(is.null(ylim))
					plot(ts,ylim=c(min(ts,na.rm=TRUE),max(ts,na.rm=TRUE)*ytop),main=main,xlab=xlab,ylab=ylab,lwd=lwd_original,col=col_original,lty=lty_original,...)
				else
					plot(ts,ylim=ylim,main=main,xlab=xlab,ylab=ylab,lwd=lwd_original,col=col_original,lty=lty_original,...)
				if(showWarnings)
